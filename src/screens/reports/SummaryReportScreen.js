...
export default function SummaryReportScreen() {
  ...
  useEffect(() => {
   ...
   function refreshReportByDate(startDate, endDate){
    ...
    ReportService.sales((sales) => revenueSummaries[0].value = sales, startDate, endDate);
    ReportService.profit((profit) => revenueSummaries[1].value = profit, startDate, endDate);
    ReportService.salesByPayment((credit) => revenueSummaries[2].value = credit, startDate, endDate, "credit")


    // 2. Refresh payment reports 
    ReportService.salesByPayment((cashSales) => paymentMethod[0].value = cashSales, startDate, endDate, "cash")
    ReportService.salesByPayment((mobileMoneySales) => paymentMethod[1].value = mobileMoneySales, startDate, endDate, "mobile")
    ReportService.salesByPayment((otherSales) => paymentMethod[2].value = otherSales, startDate, endDate, "other")
    ...
    ReportService.fastMovingStock((fastMoving) => stockSummaries[2].value = fastMoving, startDate, endDate);
    ReportService.slowMovingStock((slowMoving) => stockSummaries[3].value = slowMoving, startDate, endDate);
  }
  ...
  return (
    <View>
      ...
      <View style={styles.row}>
       {revenueSummaries.map((item)=> <RenderReportItem title={item.title} value={money(item.value)} titleColor={item.color}/>
        )}
      </View>
      ...
      <View style={styles.row}>
       {paymentMethod.map((item)=> <RenderReportItem title={item.title} value={money(item.value)} titleColor={item.color}/>
        )}
      </View>
      ...
      <View style={styles.row}>
       {stockSummaries.map((item)=> <RenderReportItem title={item.title} value={number(item.value)} titleColor={item.color}/>
        )}
      </View>
      </View>
    </View>
