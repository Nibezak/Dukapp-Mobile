export default {
  today_insights: "Insights",
  last_4_orders: 'Recent Sales',
  place_an_order: 'GURISHA',
  place_order_first: 'BANZA UGURISHE',
  create_item_in_stock: 'Shyiramo ikigurishwa',
  order_deleted: 'Icyatumijwe cyasibwe',
  screen_header: 'Gucunga byihuse kandi byoroshye',
  screen_header_stock: 'Tangira ushyira ibigurishwa mu bubiko',
  screen_header_customers: 'Gumana abakiriya bawe',
  screen_description:
    "Cunga ibicuruzwa mu bubiko bwawe, ubare ibyinjira n'inyungu yawe, kandi ushireho inyemezabwishyu",
};
