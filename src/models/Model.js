...
export default class Model {
    ...
    like(attribute, value) {
        if (this.conditions === null) {
            ...
            this.conditions =
                this.conditions + " AND " + attribute + ` LIKE '%` + value + `%'`;
        }
    ...
    async save() {
        // Run against db
        return this.db
            .statement(this.queryString, this.queryParameters)
            .then((res) => {
     ...
     */
    async dropTable() {
        return await this.db.statement(
    ...
    buildUpdateQuery(params) {
        ...
        this.queryString =
            `UPDATE ` + this.getTableName() + ` SET ` + attributes + `;`;
    ...
    getKeyValuePair(params) {
        ...
        return Object.keys(params)
            .map((key, value) => {
                ...
                values[value] = !isNaN(values[value]) ?
                    values[value] :
                    `'` + values[value] + `'`;
    ...
    createTable() {
        throw (
            "please implement createTable() method for model " + this.constructor.name
        );
    }
